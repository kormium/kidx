    ...
    Database(request.result, logger).also { database ->
        if (versionChangeEvent != null) {
            ...
            transaction.initialize(
                database,
                versionChangeEvent.oldVersion.toInt(),
                versionChangeEvent.newVersion.toInt(),
            )
    ...
    public suspend fun <T> transaction(
        ...
        val transaction = Transaction(
            ensureDatabase().transaction(
                store.map { it.toJsString() }.toJsArray(),
                "readonly",
...
private fun transactionOptions(durability: Durability): IDBTransactionOptions = jso {
